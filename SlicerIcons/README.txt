...
Palette files are in GIMP (.gpl) format, and can be imported into Inkscape by putting them in the
...
/*-------------------------------------------------------------------------------------------------*/
  ...
  To CHANGE COLORS FOR THEME/STATE in the LAYEREDSTYLES method:
   * Requires some text editing of Icon SVG's XML
   * Requires icon designer to create icon version for each theme/state
   * creates larger SVG files 
   * but uses simpler swapping logic.
  /*------------------------------------------------------------*/
...
EITHER: When theme or state changes, the SVG's xml should be modified so that ONLY the appropriate
THEME/STATE layer is changed to visibility="visible". If the current THEME/STATE is LightThemeEnabled,
the default XML can be sent to the SvgRenderer.

OR: Tupdate each icon to a new THEME/STATE, read the original SVG's XML and
use ONLY the group with the relevant THEME/STATE name and override its visibility attribute.


  CREATING NEW ICONS IN THEME/STYLE LAYERS

    1. Each Icon's design must be contained in a single 'layer' or 'group' <g /g>.
    Each SVG file describes FOUR layers (or groups), each NAMED and STYLED (using palettes)
    for a THEME/STATE permutation.
    The four layers/groups should be precisely overlapping so only one at a time is visible in the
    stacking order.

    IF the icon has been created in Inkscape, each can be created in a layer,
    and layers can be named as below, to make subsequent editing the SVG'S XML easier:
       LightThemeEnabledLayer
       LightThemeDisabledLayer
       DarkThemeEnabledLayer
       DarkThemeDisabledLayer
    Inkscape represents layers as groups in an exported SVG's XML, each tagged with <g /g>.
    ...
    2. If NOT using Inkscape to design, the Icon's SVG's XML may need to be edited to isolate each
    layer inside its own THEME/STATE group <g /g>.

    3. As noted, the Icon SVG's XML must also be edited to set the id inside each
...
ANY THEME/STATE -> Light/Disabled
  ...
  1. If its inline "stroke" style attribute that does NOT have a value of stroke:none,
     ...
     of the style specification, e.g. style="....;stroke:#CCCCCC;" to override.
  ...
  2. If its inline "fill" style attribute does NOT have a value of fill:none,
        /* capture cases where icon uses a filled curve instead of stroke */
	     ...
	     of the style specification, e.g. style="....;fill:#CCCCCC;" to override.
	ELSE
             APPEND new fill color with value=(corresponding FILL Light/Disabled color) at the END
	     of the style specification, e.g. style="....;fill:#F5F5FF;" to override.
  }
